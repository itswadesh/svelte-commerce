...
;(function () {
	...
	function buildGeneratingRow() {
		...
		status.textContent = arrivedVariants < expectedVariants ? 'Generating ' + expectedVariants + ' variants...' : 'Done'
			...
			for (const m of mutations) {
				if (m.target?.closest?.('[data-impeccable-variants="' + sessionId + '"]')) {
					const childAdds = Array.from(m.addedNodes)
						.map((n) =>
							n.nodeType === 1 ? n.tagName + (n.dataset?.impeccableVariant ? '[variant=' + n.dataset.impeccableVariant + ']' : '') : n.nodeType
						)
						.join(',')
				...
				for (const n of m.addedNodes) {
					if (
						n.nodeType === 1 &&
						(n.matches?.('[data-impeccable-variants="' + sessionId + '"]') || n.querySelector?.('[data-impeccable-variants="' + sessionId + '"]'))
					) {
			...
			switch (msg.type) {
				...
				case 'done':
					...
					setTimeout(() => {
						...
						showToast("Variants ready. If the picked element isn't visible, retrace the path that revealed it — they'll appear automatically.", 15000)
					}, 2000)
					break
				case 'error':
...
void main() {
  ...
  gl_FragColor = vec4(mix(base, dotLayer, band), 1.0);
	...
	async function showShaderOverlay(el, blob, rect) {
		...
		const gl = canvas.getContext('webgl', { premultipliedAlpha: false, preserveDrawingBuffer: false }) || canvas.getContext('experimental-webgl')
		if (!gl) {
