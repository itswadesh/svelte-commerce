...
# Feeds kit.version.name (see svelte.config.js), shown in the footer. `.git` is copied in, so the
...
# would desync the `__sveltekit_<hash>` global and break every page in prod (see svelte.config.js).
