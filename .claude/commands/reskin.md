...
`reskin.config.json` supplies `out`, `concurrency`, and `engine`; do not pass those flags unless the
user explicitly asks for something different in this message.
...
After it finishes, report concisely:
- Where the theme/output files landed.
