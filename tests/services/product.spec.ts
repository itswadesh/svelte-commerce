...
test('Test: fetch product by id or slug', async ({ request }) => {
	...
	const res2 = await request.get(`${API_URL}es/products/${product_slug}?store=${storeId}`)
...
test('Test: fetch products of category', async ({ request }) => {
	...
	const res = await request.get(
		`${API_URL}es/products?categories=${category_slug}&zip=${zip}&store=${storeId}&${query}`
	)
...
test('Test: fetch related products from category ', async ({ request }) => {
	...
	const res = await request.get(
		`${API_URL}es/products?categories=${category_slug}&store=${storeId}`
	)
