...
describe('ProductCard Component', () => {
	it('should be importable', async () => {
		const ProductCard = await import('$lib/components/product-catalogue/product-card.svelte')
		expect(ProductCard).toBeDefined()
		expect(ProductCard.default).toBeDefined()
	})

	it('should have proper component structure', async () => {
		const component = await import('$lib/components/product-catalogue/product-card.svelte')
