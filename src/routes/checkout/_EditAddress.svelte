    ...
    <div v-if="address">
      ...
      >
        ...
        <div class="flex flex-col-reverse col-span-2 md:col-span-1">
          ...
          <div class="flex flex-col-reverse col-span-2 md:col-span-1">
            <input type="text" bind:value="{address.state}" placeholder="" disabled />
            <span class="my-2">State</span>
          </div>

          <div class="flex flex-col-reverse col-span-2 md:col-span-1">
            <input type="text" bind:value="{address.country}" placeholder="" disabled />
        ...
        <div class="w-full mt-8">
          ...
          >
            ...
            <Button>
              ...
              {:else}
              <span class="text-base"> ADD ADDRESS</span>
              {/if}
            </Button>
          </div>
          </div>
 ...
 $:   user = $session.user
  onMount(()=>{
    ...
    async function submit() {
      if (!address) return console.log('Please enter address')
      // address.phone =
      //   address.phone && address.phone.replace(/^0+/, '')
      if (!address.address) return console.log('Please enter address')
      const msg = id === 'new' ? 'Address Added' : 'Address Updated'
      try {
        iconloading = true
        if (!address?.id) address.id = 'new'
        address = (
          await post('/addresses/save',address          )
        ).data.saveAddress
        console.log(msg)
        if (prev)
          goto(`/${prev}`)
