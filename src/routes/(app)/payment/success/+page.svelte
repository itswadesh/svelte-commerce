...
{#if data.order}
	<div class="container mx-auto w-full max-w-6xl px-4 py-5 pb-10 text-gray-800 sm:px-10 md:py-10 ">
		...
		<div class="text-center">
			...
			<ul class="mx-auto mb-2 flex max-w-max flex-col gap-2">
				<li class="flex items-center gap-2 text-sm">
					<p class="w-40 text-right">Order Number :</p>
				...
				<li class="flex items-center gap-2 text-sm">
					<p class="w-40 text-right">Order place on :</p>
			...
			<p class="mb-5 text-sm sm:mb-10">
				A confirmation e-mail will be sent to the e-mail address that you specified in Order
				details.
			</p>

			<div class="mb-5 flex flex-wrap items-center justify-center gap-5 sm:mb-10">
				{#if data.order.seatsBooked}
					<a
				...
				<a href="/" rel="noopener" aria-label="Click to route home" sveltekit:prefetch>
		...
		<div
			...
			<div class="sm:w-1/2">
				{#if data.order.items?.length > 0}
					<div class="mb-5">
						<h6 class="border-b border-dashed border-gray-400 pb-2 text-base font-bold sm:text-lg">
							Item Details
						</h6>
						...
						<div class="itmes-start flex flex-col">
							{#each data.order.items as item, ix}
								<a
									href="/product/{item.slug}?id={item.pid}"
												...
												<!-- {#if store.isFnb}
													<div>
														...
														{:else if item.foodType === 'N' || item.foodType === 'E'}
															<LazyImg src="/product/non-veg.png" alt="veg" width="20" height="20" class="h-5 w-5" />
														{:else}
															<LazyImg src="/product/other.png" alt="veg" width="20" height="20" class="h-5 w-5" />
														{/if}
													</div>
												{/if} -->
											</div>
