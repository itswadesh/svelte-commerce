	...
	<div class="container mx-auto w-full max-w-6xl">
		...
		<ul class="flex flex-col gap-5 text-justify text-sm sm:text-base">
			...
			<li>
				...
				<p class="text-zinc-500">
					{$page.data?.store?.websiteName}’s trademarks and trade dress may not be used in
					connection with any product or service that is not {$page.data?.store?.websiteName}’s, in
					any manner that is likely to cause confusion among customers, or in any manner that
					disparages or discredits {$page.data?.store?.websiteName}. All other trademarks not owned
					by {$page.data.store?.websiteName} or its subsidiaries that appear on this site are the property
					of their respective owners, who may or may not be affiliated with, connected to, or sponsored
					by
			...
			<li>
				...
				<p class="text-zinc-500">
					...
					(including images, text, page layout, or form) of {$page.data?.store?.websiteName} and our
					associates without express written consent. You may not use any meta tags or any other “hidden
					text” utilizing
					...
					{$page.data?.store?.websiteName} logo or other proprietary graphic or trademark as part of
			...
			<li>
				...
				<p class="text-zinc-500">
					...
					material, and unless we indicate otherwise, you grant {$page.data?.store?.websiteName} and
					its associates a nonexclusive, royalty-free, perpetual, irrevocable, and fully sub-license-able
			...
			<li>
				...
				<p class="mb-5 text-zinc-500">
					{$page.data?.store?.websiteName} and its associates attempt to be as accurate as possible.
					However,
			...
			<li>
				...
				<p class="text-zinc-500">
					...
					{$page.data?.store?.websiteName} shall be submitted to confidential arbitration in Odisha,
