...
<section class="w-full h-full pl-2 sm:pl-8 sm:pr-2 text-gray-800 tracking-wide ">
	...
	{#if wishlists?.count > 0}
		{#each wishlists?.data as wishlist}
			<div
				class=" relative p-4 my-2 sm:my-5 transition duration-300 bg-white bwishlist-t bwishlist-gray-300 rounded-md md:shadow-md bwishlist ">
				<div class="flex justify-between items-center">
					<!-- Details section start  -->
					<div class="sm:ml-4">
						<a href="{`/wishlists/${wishlist.id}`}" class="cursor-pointer">
							<span class="text-lg font-semibold tracking-wider md:text-2xl">
								{wishlist.wishlistNo}</span>
						</a>
						...
						<div class="mt-2 relative inline-block text-left">
							<div on:click="{toggle}" class="font-semibold cursor-pointer">Wishlist Status</div>
						...
						<!-- Email div start  -->
						<div v-if="wishlist.user" class="my-1">
						...
						<div class="flex my-2 text-sm cursor-pointer item-center">
							...
							<span class="ml-2"
								>Pay by
								<span class="font-semibold">{wishlist.paymentMode}</span></span>
						</div>
						<!-- Pay by case div end -->
						...
						<div>
							<div class="xl:flex">
								<div
									...
									{#if wishlist && wishlist.amount}
										<span class="">
											{currency(wishlist.amountotal)}
										</span>
									{/if}
								</div>
