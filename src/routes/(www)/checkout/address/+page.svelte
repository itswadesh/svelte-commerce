...
import { ChevronRight, LockKeyhole, MapPin, Pencil, ShoppingBag, Truck } from '@lucide/svelte'
		...
		<div class="mb-12">
			<div class="flex items-center justify-center space-x-4 sm:space-x-12">
				...
				<div class="flex items-center text-primary">
					<div class="flex h-7 w-7 items-center justify-center rounded-full bg-primary text-[11px] font-bold text-white tracking-tight">2</div>
			...
			<div class="grid gap-8 lg:grid-cols-[1fr_400px]">
				...
				<div class="space-y-6">
						...
						<div class="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
							<div class="flex items-center justify-between border-b border-gray-50 px-5 py-4">
								<div class="flex items-center space-x-3">
									<h2 class="text-base font-bold uppercase tracking-widest text-gray-900" style="font-family: 'Montserrat', sans-serif;">Contact Details</h2>
								</div>
								{#if cartState.cart.email && !addressModule.editEmail && !userState.user?.userId}
									<Button
										...
										variant="ghost"
										size="sm"
										class="h-8 text-[10px] font-bold uppercase tracking-widest text-primary hover:bg-primary/5 transition-all active:scale-95"
									>
							...
							{#if isEmailOk && isPhoneOk && !addressModule.editEmail}
								<div class="grid grid-cols-1 gap-6 p-6 sm:grid-cols-2 transition-all duration-500">
							...
							{:else if !isEmailOk || !isPhoneOk || addressModule.editEmail}
								<form
									class="space-y-4 p-5 transition-all duration-500"
                      ...
                      Email address {#if !addressModule.isEmailRequired}<span class="text-gray-400">(optional)</span>{/if}
									...
									<div class="flex justify-end space-x-3 pt-2">
										...
										<Button
											...
											class="min-w-[120px] transition-all active:scale-95"
										>
					...
					{#if isEmailOk && isPhoneOk}
						<div class="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:shadow-md">
							{#if cartState.cart.shippingAddress}
								<div class="">
									<div class="flex items-center justify-between border-b border-gray-50 px-5 py-4">
										<h2 class="text-base font-bold uppercase tracking-widest text-gray-900" style="font-family: 'Montserrat', sans-serif;">Delivery Address</h2>
									...
									{:else}
										<div class="p-6 transition-all duration-500">
											...
											<div class="space-y-1 text-sm text-gray-600 leading-relaxed">
												...
												<p>
													{cartState.cart.shippingAddress?.countryCode} {cartState.cart.shippingAddress?.zip}
												</p>
												<p class="mt-4 pt-4 border-t border-gray-50 font-medium">
													<span class="text-[10px] font-bold uppercase tracking-tighter text-gray-400 mr-2">Phone</span>
									...
									<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
										...
										{#if !userState?.user?.role}
											<button
												...
												class="text-[10px] font-bold uppercase tracking-widest text-primary hover:underline text-left transition-all active:scale-95"
											>
                    ...
                    {#if addressModule.loadingForSaveToCart && addressModule.currentAddressType == 'shipping'}
										  <Skeleton class="h-[100px] w-full rounded-lg" />
									  {:else}
										<div class="flex flex-col items-center justify-center py-8 text-center bg-gray-50 rounded-lg border border-dashed border-gray-200 transition-all duration-300 hover:bg-gray-100/50">
											<p class="text-sm text-gray-500 mb-6">No shipping address selected</p>
											<Button type="button" variant="default" class="bg-primary hover:bg-black text-[10px] font-bold uppercase tracking-widest px-8 transition-all active:scale-95" onclick={addressModule.handleAddNewAddress}>Add New Address</Button>
										</div>
                    {/if}
									</form>
								</div>
						...
						{#if !addressModule.isBillingAddressSameAsShipping}
							<div class="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:shadow-md">
								<div class="">
									<div class="flex items-center justify-between border-b border-gray-50 px-5 py-4">
										<h2 class="text-base font-bold uppercase tracking-widest text-gray-900" style="font-family: 'Montserrat', sans-serif;">Billing Address</h2>
                    {#if !addressModule.loadingForSaveToCart}
										  <Button
                  ...
                  {#if addressModule.loadingForSaveToCart && addressModule.currentAddressType == 'billing'}
									...
									{:else}
										<div class="p-8 text-center bg-gray-50 transition-all duration-500">
						...
						{/if}
						<div class="flex items-center justify-start gap-2 bg-gray-50 p-4 rounded-lg border border-gray-100 transition-all hover:bg-gray-100">
							<Checkbox
							...
							/>
							<label for="isBillingAddressSameAsShipping" class="text-xs font-bold uppercase tracking-tight text-gray-700 cursor-pointer">Billing address same as shipping</label>
						</div>
					{/if}
				</div>
					...
					<div class="space-y-4 rounded-lg border border-gray-100 p-6 bg-white shadow-sm">
						...
						{:else}
							<div class="space-y-4">
								...
								<div class="mt-6 flex items-center justify-center gap-2 py-3 px-4 rounded-md border border-gray-100 bg-gray-50/50 transition-all hover:bg-gray-100/80">
								...
								{#if isPhoneOk && isEmailOk && cartState.cart.shippingAddress && !addressModule.editAddress}
									...
									>
										{#if addressModule.loadingForCheckout}
											<LoadingDots />
										{:else}
											<span>Continue to Payment</span> <ChevronRight class="ml-2 size-4 transition-transform duration-300 group-hover:translate-x-1" />
										{/if}
									</Button>
								{/if}
							</div>
