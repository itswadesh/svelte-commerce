            ...
            {/if} -->
					...
					{#if !page.url.pathname.startsWith('/checkout')}
						<div class="relative" role="navigation">
							<a class="flex items-center rounded-full lg:hidden transition-transform duration-300 hover:scale-110 active:scale-90" href="/checkout/cart" aria-label="View Cart">
								<svg
									width="24"
									height="24"
							...
							<button
								class="hidden items-center rounded-full lg:flex transition-transform duration-300 hover:scale-110 active:scale-90"
							...
							{#if cartState.isOpen}
								<!-- close cart backdrop -->
								<button
									class="fixed inset-0 z-50 h-svh w-full overflow-hidden bg-black bg-opacity-30 scrollbar-none transition-opacity duration-300"
								...
								<div
									class="shadow-3xl fixed right-0 top-0 z-[10000000] h-screen w-full overflow-y-auto bg-white sm:w-[37.5%] transition-all duration-500 ease-out-expo"
								...
								>
									<div class="relative z-50 flex h-full flex-col justify-between bg-white p-4">
										<div class="sm:mx-3">
											<h2 class="mb-4 mt-4 text-xl font-bold uppercase tracking-widest text-gray-900" style="font-family: 'Montserrat', sans-serif;">My Shopping Cart</h2>
											<button class="absolute right-4 top-4 rounded-full p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-900 transition-all active:scale-75" aria-label="close cart" onclick={navModule.closeCartSidebar}>
												<X class="size-5" />
											</button>

											{#each cartState.cart?.lineItems || [] as _, i}
												<div class="transition-all duration-300 hover:bg-gray-50 rounded-lg">
													<CartItem bind:cartProduct={cartState.cart.lineItems[i]} removeItem={navModule.removeCartItem} />
												</div>
											{/each}
										</div>

										<div class="sticky bottom-0 {cartState.cart?.lineItems?.length > 0 ? '-mx-4 bg-gray-50 border-t border-gray-100 shadow-[0_-10px_30px_-15px_rgba(0,0,0,0.1)]' : ''} py-6 sm:px-4">
											<div class="space-y-1">
												...
												{:else}
													<div class="flex min-h-[80vh] flex-col items-center justify-center gap-3 bg-white">
														...
														<span class="max-w-xs px-2 text-center text-xs font-medium text-gray-500 leading-relaxed"
											...
											{#if cartState?.cart?.total >= 0 && cartState.cart?.lineItems?.length > 0}
												<div class="mx-4 mt-6 flex justify-between gap-3">
													...
													<Button
														...
														class="w-[48%] py-6 text-[10px] font-bold uppercase tracking-widest bg-primary hover:bg-black transition-all active:scale-95"
													>
														Proceed
													</Button>
												</div>
					...
					<div class="flex h-full items-center">
						{#if userState?.user?.role}
							<DropdownMenu.Root>
								<DropdownMenu.Trigger aria-label="User Profile" class="transition-transform duration-300 hover:scale-110 active:scale-90">
									<div class="flex items-center justify-center">
										{#if userState.user?.avatar}
											<LazyImg
												...
												class="h-8 w-8 rounded-full border border-gray-100 object-cover object-top shadow-sm"
											/>
										{:else}
