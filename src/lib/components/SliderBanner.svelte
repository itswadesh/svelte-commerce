		...
		<div class="container mx-auto flex h-full max-h-max max-w-6xl flex-col justify-between gap-5">
			...
			<div class="mt-auto flex flex-wrap items-start justify-between gap-4 md:items-end sm:gap-10">
				<div class="flex flex-col sm:flex-row gap-4 tracking-wide text-white sm:gap-10">
					...
					{:else}
						<div
							class="flex h-16 w-16 p-2 flex-shrink-0 flex-col items-center justify-center rounded-full border border-gray-300 bg-gray-100 text-xs text-gray-400 text-center gap-1 shadow-md sm:h-24 sm:w-24 overflow-hidden">
							<svg
								xmlns="http://www.w3.org/2000/svg"
								fill="none"
								viewBox="0 0 24 24"
					...
					<div class="flex-1 flex flex-col gap-2">
						...
						{#if data.address?.address || data.address?.landmark || data.address?.city || data.address?.country || data.address?.zip}
							<div class="flex items-start gap-1 text-sm text-gray-100">
								<svg
									xmlns="http://www.w3.org/2000/svg"
...
<Gallery bind:showPhotosModal="{showPhotosModal}" images="{data.banners}" title="{data?.businessName}" />
