...
<div
	class="group relative col-span-1 block w-full overflow-hidden hover:bg-white hover:shadow-lg sm:w-48 sm:flex-shrink-0"
	...
	<a
		...
		{#if product.new || product.tags?.length}
			<div class="absolute top-2 left-2 flex flex-col gap-1">
				{#if product.new}
					<div
						class="text-rem max-w-max rounded bg-red-500 py-0.5 px-1.5 text-center font-bold uppercase tracking-widest text-white">
						NEW
					</div>
				{/if}
				...
				{#if product.tags?.length}
					{#each product.tags as tag}
						<div
							class="text-rem max-w-max rounded py-0.5 px-1.5 text-center font-bold uppercase tracking-widest text-white"
				...
				<a
					...
					<div class="mb-1.5 flex items-center justify-between">
						<h1 class="font-semibold">
							{product.brand?.name || product.brandName || '_'}
						</h1>
					...
					<h2 class="overflow-hidden overflow-ellipsis whitespace-nowrap text-sm">
						{#if product.name}
							{product.name}
						{:else}
							_
	...
	<div class="fixed inset-0 z-[100] h-screen w-full">
		...
		<div
			...
			<div class="h-full flex-shrink-0">
				<h1 class="p-4 text-center font-bold uppercase sm:text-lg">Similar Products</h1>
								...
								<div class="p-4">
									...
									<div class="mt-2.5 flex flex-wrap items-baseline justify-start leading-4">
										...
										{#if relatedProduct.discount > 0}
											<span class="mr-1 whitespace-nowrap text-xs">
