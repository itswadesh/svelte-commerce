...
<div>
	{#if title}
		<button
			...
			<h6 class="flex-1 text-sm font-normal text-primary-500 capitalize">{title}</h6>
			...
			<svg
				...
				{showData ? 'text-secondary-500 transform -rotate-180' : ''}">
				<path
					fill-rule="evenodd"
	...
	{#if showData && filteredTerms?.length}
		<ul transition:slide="{{ duration: 300 }}" class="lg:flex flex-col hidden">
