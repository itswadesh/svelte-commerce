import * as nopcommerce from '@misiki/nopcommerce-connector'
...
// nopCommerce-only connector. Everything the backend itself can answer for — catalogue, cart, checkout,
// auth, orders, addresses — and honest empty states for what nopCommerce has no equivalent of live in
// @misiki/nopcommerce-connector (>= 0.5.1). What remains here is what only this storefront can answer.
export * from '@misiki/nopcommerce-connector'

// Lets the hooks `init` verify the selected connector matches the PUBLIC_NOPCOMMERCE_* env, and names
// this backend's row in the CONNECTORS table in init.ts.
export const connectorName = 'nopcommerce'

// Store identity — name, logo, favicon, currency, menus, plugin toggles, theme variables — has no
// nopCommerce equivalent, so the connector reads it from here: default-store.json merged under the
...
;(nopcommerce as { setStaticStore?: (fn: typeof staticStoreConfig) => void }).setStaticStore?.(staticStoreConfig)

// Local data first — countries, currencies, plugin toggles, the rest of the store record — then the
// net: nothing in nopCommerce mode may fall through to a Litekart REST path, because that API is neither
// installed nor running. See rest-guard.ts.
serveRestLocally(localStoreData)
blockRestFallbacks(nopcommerce.BaseService, 'nopcommerce')
