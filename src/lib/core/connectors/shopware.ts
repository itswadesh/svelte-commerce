import * as shopware from '@misiki/shopware-connector'
...
// Shopware-only connector. Everything the backend itself can answer for — catalogue, cart, checkout,
// auth, orders, addresses — and honest empty states for what Shopware has no equivalent of live in
// @misiki/shopware-connector (>= 0.4.1). What remains here is what only this storefront can answer.
export * from '@misiki/shopware-connector'

// Lets the hooks `init` verify the selected connector matches the PUBLIC_SHOPWARE_* env, and names
// this backend's row in the CONNECTORS table in init.ts.
export const connectorName = 'shopware'

// Store identity — name, logo, favicon, currency, menus, plugin toggles, theme variables — has no
// Shopware equivalent, so the connector reads it from here: default-store.json merged under the
...
;(shopware as { setStaticStore?: (fn: typeof staticStoreConfig) => void }).setStaticStore?.(staticStoreConfig)

// Local data first — countries, currencies, plugin toggles, the rest of the store record — then the
// net: nothing in Shopware mode may fall through to a Litekart REST path, because that API is neither
// installed nor running. See rest-guard.ts.
serveRestLocally(localStoreData)
blockRestFallbacks(shopware.BaseService, 'shopware')
