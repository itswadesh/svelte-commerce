import * as csCart from '@misiki/cs-cart-connector'
...
// CS-Cart-only connector. Everything the backend itself can answer for — catalogue, cart, checkout,
// auth, orders, addresses — and honest empty states for what CS-Cart has no equivalent of live in
// @misiki/cs-cart-connector (>= 0.4.1). What remains here is what only this storefront can answer.
export * from '@misiki/cs-cart-connector'

// Lets the hooks `init` verify the selected connector matches the PUBLIC_CS_CART_* env, and names
// this backend's row in the CONNECTORS table in init.ts.
export const connectorName = 'cs-cart'

// Store identity — name, logo, favicon, currency, menus, plugin toggles, theme variables — has no
// CS-Cart equivalent, so the connector reads it from here: default-store.json merged under the
...
;(csCart as { setStaticStore?: (fn: typeof staticStoreConfig) => void }).setStaticStore?.(staticStoreConfig)

// Local data first — countries, currencies, plugin toggles, the rest of the store record — then the
// net: nothing in CS-Cart mode may fall through to a Litekart REST path, because that API is neither
// installed nor running. See rest-guard.ts.
serveRestLocally(localStoreData)
blockRestFallbacks(csCart.BaseService, 'cs-cart')
