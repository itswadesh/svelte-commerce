import * as evershop from '@misiki/evershop-connector'
...
// EverShop-only connector. Everything the backend itself can answer for — catalogue, cart, checkout,
// auth, orders, addresses — and honest empty states for what EverShop has no equivalent of live in
// @misiki/evershop-connector (>= 0.4.1). What remains here is what only this storefront can answer.
export * from '@misiki/evershop-connector'

// Lets the hooks `init` verify the selected connector matches the PUBLIC_EVERSHOP_* env, and names
// this backend's row in the CONNECTORS table in init.ts.
export const connectorName = 'evershop'

// Store identity — name, logo, favicon, currency, menus, plugin toggles, theme variables — has no
// EverShop equivalent, so the connector reads it from here: default-store.json merged under the
...
;(evershop as { setStaticStore?: (fn: typeof staticStoreConfig) => void }).setStaticStore?.(staticStoreConfig)

// Local data first — countries, currencies, plugin toggles, the rest of the store record — then the
// net: nothing in EverShop mode may fall through to a Litekart REST path, because that API is neither
// installed nor running. See rest-guard.ts.
serveRestLocally(localStoreData)
blockRestFallbacks(evershop.BaseService, 'evershop')
