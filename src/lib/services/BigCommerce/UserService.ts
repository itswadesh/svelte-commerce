import { delBySid, getBigcommerceApi, postBigCommerceApi } from '$lib/utils/server'
...
export const fetchMeData = async ({ origin, storeId, server = false, sid = null }: any) => {
	try {
		...
		const response = await getBigcommerceApi(`auth`, null, sid)
...
export const signupService = async ({
	...
	try {
		...
		const response = await postBigCommerceApi(`customers`, {
...
export const loginService = async ({
	...
	try {
		...
		const response = await postBigcommerceApi(`auth`, {
			email,
			password
		})
		res = response.customer
