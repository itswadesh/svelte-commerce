...
2. Point `kitcommerce.config.ts` at it:
   ...
   ```ts
   export * as services from '@misiki/shopware-connector'
   ```

3. Give the connector its base URL. There is no `PUBLIC_SHOPWARE_API_URL` handling in
   ...
   statics `@misiki/shopware-connector` exposes on its `BaseService`.

4. You do not need an override module. `@misiki/shopware-connector` carries its own
