...
2. Point `kitcommerce.config.ts` at it:
   ...
   ```ts
   export * as services from '@misiki/spree-connector'
   ```

3. Give the connector its base URL. There is no `PUBLIC_SPREE_API_URL` handling in
   ...
   statics `@misiki/spree-connector` exposes on its `BaseService`.

4. You do not need an override module. `@misiki/spree-connector` carries its own
