...
2. Point `kitcommerce.config.ts` at it:
   ...
   ```ts
   export * as services from '@misiki/virto-connector'
   ```

3. Give the connector its base URL. There is no `PUBLIC_VIRTO_COMMERCE_API_URL` handling in
   ...
   statics `@misiki/virto-connector` exposes on its `BaseService`.

4. You do not need an override module. `@misiki/virto-connector` carries its own
