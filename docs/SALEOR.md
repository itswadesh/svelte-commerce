...
`@misiki/saleor-connector` still calls the Litekart API endpoint `/api/stores/public-details` for
...
`src/lib/core/connectors/saleor.ts` re-exports the whole connector and replaces only
`StoreService` — the Saleor connector's `PageService` already serves static dummy pages, so it
needs no override:
...
| everything else Litekart-REST | Never requested: collection reads resolve empty, single-record reads and writes throw — see `no-litekart-rest.ts` |
...
- **`http proxy error: api/... ECONNREFUSED` in dev** — something is calling a Litekart REST
  ...
  In Saleor mode that service needs a static override in `src/lib/core/connectors/saleor.ts`.
