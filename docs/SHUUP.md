...
2. Point `kitcommerce.config.ts` at it:
   ...
   ```ts
   export * as services from '@misiki/shuup-connector'
   ```

3. Give the connector its base URL. There is no `PUBLIC_SHUUP_API_URL` handling in
   ...
   statics `@misiki/shuup-connector` exposes on its `BaseService`.

4. You do not need an override module. `@misiki/shuup-connector` carries its own
