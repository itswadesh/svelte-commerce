...
3. Give the connector its base URL. There is no `PUBLIC_SPRYKER_API_URL` handling in
   ...
   statics `@misiki/spryker-connector` exposes on its `BaseService`.

4. You do not need an override module. `@misiki/spryker-connector` carries its own
