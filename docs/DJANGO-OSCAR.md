...
3. Give the connector its base URL. There is no `PUBLIC_DJANGO_OSCAR_API_URL` handling in
   ...
   statics `@misiki/oscar-connector` exposes on its `BaseService`.

4. You do not need an override module. `@misiki/oscar-connector` carries its own
