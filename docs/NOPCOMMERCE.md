...
3. Give the connector its base URL. There is no `PUBLIC_NOPCOMMERCE_API_URL` handling in
   ...
   statics `@misiki/nopcommerce-connector` exposes on its `BaseService`.

4. You do not need an override module. `@misiki/nopcommerce-connector` carries its own
