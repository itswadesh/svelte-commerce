...
- Multi-backend support: Medusa, Vendure and Saleor connectors alongside Litekart and Shopify, selected from a single line in `kitcommerce.config.ts`
- Connector override modules (`src/lib/core/connectors/*`) that serve store/page metadata statically for backends with no Litekart API behind them
- Static store identity config (`default-store.json` + the `kitcommerce.config.ts` default export) for name, logo, favicon, currency, menus and theme variables
- `/sitemap.xml` route with a sitemap proxy server
- Node adapter for production builds (`npm start`)
- Checkout success page
- Mobile-responsive login modal with OTP verification and post-auth redirect
- Switchable storefront themes via `PUBLIC_STOREFRONT_THEME`, with store settings able to override it

### 🐛 Bug Fixes

- Fixed `/collections` 500 caused by filters reading state the route never populated
- Hardened the Saleor page fallback so missing page metadata no longer breaks rendering

### 🔄 Improvements

- Build no longer fails on `@misiki/litekart-connector` when a different connector is installed — Vite and the type layer now redirect that specifier to the installed `@misiki/*-connector`
- `npm run check` syncs connector types before running `svelte-check`

### 📚 Documentation

- Corrected the Get Started instructions: added the `cp .env.example .env` step and fixed the dev server URL to `http://localhost:5173`
- Documented all 26 connectors, the storefront capability matrix, and the required backend settings per platform
- Added `docs/MEDUSA.md`, `docs/SALEOR.md`, `docs/VENDURE.md` and `docs/CONNECTORS.md` setup guides, and moved the changelog to `docs/CHANGELOG.md`
