...
3. Give the connector its base URL. There is no `PUBLIC_SWELL_API_URL` handling in
   ...
   statics `@misiki/swell-connector` exposes on its `BaseService`.

4. You do not need an override module. `@misiki/swell-connector` carries its own
